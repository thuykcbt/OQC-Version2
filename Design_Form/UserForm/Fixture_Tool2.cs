...
namespace Design_Form.UserForm
{
    ...
    public partial class Fixture_Tool2 : UserControl
    {
                ...
                for (int j = 0; j <= b; j++)
                {
                    for (int i = 0; i < Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools.Count; i++)
                    {
                        if (Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools[i].ToolName == "ShapeModel")
                        {
                            ...
                            combo_master2.Items.Add("Job:" + j.ToString() + "_" + Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools[i].ToolName + ":" + i.ToString());
                        }
                        if (Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools[i].ToolName == "ShapeModel_Color")
                        {
                            combo_master.Items.Add("Job:" + j.ToString() + "_" + Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools[i].ToolName + ":" + i.ToString());
                            combo_master2.Items.Add("Job:" + j.ToString() + "_" + Job_Model.Statatic_Model.model_run.Cameras[a].Views[j].Components[d].Tools[i].ToolName + ":" + i.ToString());
                        }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            ShapeModelTool shapeModelTool1 = (ShapeModelTool)Job_Model.Statatic_Model.model_run.Cameras[a].Views[index_job].Components[d].Tools[index_follow2];
